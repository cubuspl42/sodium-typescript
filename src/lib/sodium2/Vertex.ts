...
export class Source {
    ...
    register(target: Vertex_): void {
        if (!this.registered) {
            ...
            if (this.register_ !== null)
                ...
                this.origin.increment(Vertex_.NULL);
                target.childrn.push(this.origin);
                this.deregister_ = () => {
                    this.origin.decrement(Vertex_.NULL);
    ...
    deregister(target: Vertex_): void {
        if (this.registered) {
...
export function describeAll(v: Vertex_, visited: Set<number>) {
...
export class Vertex_ {

    static NULL: Vertex_ = new Vertex_("user", 1e12, []);
    static collectingCycles: boolean = false;
    static toBeFreedList: Vertex_[] = [];
    id: number;
    ...
    register(target: Vertex_): boolean {
    ...
    private decRefCount(target: Vertex_): void {
        if (verbose)
    ...
    children(): Vertex_[] { return this.childrn; }

    increment(referrer: Vertex_): boolean {
        return this.incRefCount(referrer);
    }

    decrement(referrer: Vertex_): void {
    ...
    static collectCycles(): void {
        if (Vertex_.collectingCycles) {
            return;
        }
        try {
            Vertex_.collectingCycles = true;
            Vertex_.markRoots();
            Vertex_.scanRoots();
            Vertex_.collectRoots();
            for (let i = Vertex_.toBeFreedList.length - 1; i >= 0; --i) {
                let vertex = Vertex_.toBeFreedList.splice(i, 1)[0];
                vertex.free();
            }
        } finally {
            Vertex_.collectingCycles = false;
        }
    }

    static markRoots(): void {
        const newRoots: Vertex_[] = [];
        // check refCountAdj was restored to zero before mark roots
        if (verbose) {
            let stack: Vertex_[] = roots.slice(0);
        ...
        for (let i = 0; i < roots.length; i++) {
            ...
            if (roots[i].color == Color.purple) {
                ...
                if (roots[i].color == Color.black && roots[i].refCount() == 0)
                    Vertex_.toBeFreedList.push(roots[i]);
            }
        }
        roots = newRoots;
    }
    ...
    collectWhite(): void {
        if (this.color == Color.white && !this.buffered) {
            ...
            for (let i = 0; i < chs.length; i++)
                chs[i].collectWhite();
            Vertex_.toBeFreedList.push(this);
        }
    }
}
